...
impl Plugin for DistrictPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(OnEnter(GameState::InGame), (test_gen_district,));
        app.add_systems(
            Update,
            (
                grow_districts.run_if(on_timer(Duration::from_millis(500))),
                start_new_districts.run_if(on_timer(Duration::from_millis(1000))),
                draw_district_cells,
            )
...
fn test_gen_district(mut district_map: ResMut<DistrictMap>) {
...
fn start_new_districts(mut district_map: ResMut<DistrictMap>) {
...
fn grow_districts(
        ...
        .filter(|&dist| !dist.is_completed)
    {
        ...
        if new_district.cells.len() % 4 == 0 && new_district.district_type == DistrictType::Home {
            ...
            .filter(|&cell| !spawned_cells.contains(cell))
        {
            ...
            commands.spawn((
                ...
                Transform::from_xyz(
                    ...
                    cell.1 as f32 * DISTRICT_CELL_SIZE,
                    -5.0,
                ),
            ));
        }
